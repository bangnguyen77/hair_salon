class Stylist
  ...
  define_singleton_method(:find) do |id|
    found_stylist = nil
    stylist_single = DB.exec("SELECT * FROM stylists WHERE id = #{id} limit 1;")
    found_stylist = nil
    stylist_single.each() do |stylist|
      found_stylist = Stylist.new({:id => stylist.fetch('id').to_i(), :name => stylist.fetch('name')})
    end
    found_stylist
  end
